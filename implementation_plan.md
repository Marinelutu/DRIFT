...
## Technical Rules
```
These rules apply to every phase. Never violate them.

1. LENIS PACKAGE
   Install: lenis
   Do NOT install @studio-freight/lenis — that package is abandoned.

2. GSAP PLUGIN REGISTRATION
   Create /lib/gsap.ts with:
     import { gsap } from 'gsap'
     import { ScrollTrigger } from 'gsap/ScrollTrigger'
     import { Draggable } from 'gsap/Draggable'
     gsap.registerPlugin(ScrollTrigger, Draggable)
     export { gsap, ScrollTrigger, Draggable }
   Import from this file everywhere. Never import gsap plugins without registering them first.
   Never use InertiaPlugin — it is a paid GSAP plugin. Implement drag momentum manually.

3. DRAG MOMENTUM (replaces InertiaPlugin)
   On Draggable dragEnd, implement momentum decay manually:
     Record velocity from the last few drag delta values.
     Use requestAnimationFrame to apply decaying translation:
       velocity *= 0.92 each frame until Math.abs(velocity) < 0.5, then stop.
     Clamp final position within bounds.

4. 'use client' DIRECTIVE
   Every component that uses useEffect, useState, useRef, or any browser API
   (window, document, getBoundingClientRect, matchMedia) must have 'use client'
   as the absolute first line of the file, before any imports.

5. GSAP MUST ONLY RUN INSIDE useEffect
   Never call gsap, ScrollTrigger, or any GSAP method at module level or during render.
   Next.js renders on the server — window is not defined there.
   All GSAP code lives inside useEffect(() => { ... }, []).
   Always return a cleanup function that kills all ScrollTrigger instances and gsap.context.

6. NEXT.JS <Image fill> REQUIRES A SIZED PARENT
   Any component using <Image fill> must have a parent with:
     position: relative (or absolute)
     explicit width and height (not auto, not 0)
   Without this the image renders invisibly.

7. TAILWIND V4 CONFIGURATION
   Tailwind v4 has no tailwind.config.js.
   All theme tokens go inside @theme {} in globals.css, not in :root {} and not in a JS file.
   Example:
     @theme {
       --color-base: #F5F0EB;
       --color-dark: #1A1A18;
...
8. CART CONTEXT
   Create /lib/CartContext.tsx in Phase 1.
   'use client'
   Context shape: { count: number; increment: () => void }
   Implementation: useState(0), increment adds 1.
   Wrap the app in CartProvider inside /app/layout.tsx.
   Nav.tsx reads count from context for the badge.
   ProductCard.tsx calls increment from context on Quick Add click.

9. LENIS + SCROLLTRIGGER SYNC
   In SmoothScroll.tsx, inside the same useEffect that initializes Lenis:
     gsap.ticker.add((time) => { lenis.raf(time * 1000) })
     gsap.ticker.lagSmoothing(0)
   Do NOT add ScrollTrigger.scrollerProxy() — Lenis v1+ emits native scroll events
   and the proxy will break the sync.

10. FOOTER
    Build /components/Footer.tsx as part of Phase 1.
    bg #1A1A18. py-20 px-8.
    Four columns (desktop), stacked (tablet/mobile):
      Col 1 — Logo "DRIFT" Playfair Display italic, color #F5F0EB. Below: tagline "Wear the Moment." DM Sans 13px muted.
      Col 2 — "Shop" heading DM Sans 11px tracked caps #C4622D. Links: Collections · Lookbook · New In · Sale — DM Sans 14px rgba(245,240,235,0.6), hover white.
      Col 3 — "Company" heading same style. Links: About · Sustainability · Careers · Contact.
      Col 4 — "Follow" heading same style. Links: Instagram · TikTok · Pinterest — each opens # href.
    Bottom bar (border-top 1px rgba(245,240,235,0.1), mt-16, pt-6):
      Left: "© 2025 DRIFT. All rights reserved." DM Sans 12px rgba(245,240,235,0.35).
      Right: "Wear the Moment." DM Sans 12px italic rgba(245,240,235,0.35).
```
...
Install:
- gsap (ScrollTrigger, Draggable — registered via /lib/gsap.ts per Technical Rules)
- lenis
- framer-motion
- @next/font
...
Create /lib/gsap.ts per Technical Rule 2.
Create /lib/CartContext.tsx per Technical Rule 8.
...
/app/layout.tsx
Import SmoothScroll, Nav, and CartProvider.
...
CartProvider wraps the app. SmoothScroll wraps children inside CartProvider.

Build /components/Footer.tsx per Technical Rule 10.
```
...
GSAP Draggable on rail track:
  ...
  on dragEnd: remove class. Apply manual momentum decay per Technical Rule 3.
    Snap to nearest card after momentum settles: Math.round(finalX / 340) * 340, clamped within bounds.
